...
public class SongH2Service implements SongRepository {
    ...
    public Song addSong(Song song) {
        ...
        Song savedSong = db.queryForObject("select*from playlist where songName=? and lyricist=? and singer=? and musicDierctor",new SongRowMapper(), song.getSongName(),
        song.getLyricist(),song.getSinger(),song.getMusicDirector());
        return savedSong;
    }
    ...
    public Song updateSongById(int songId, Song song) {
        ...
        if (song.getLyricist() != null) {
            db.update("update playlist set lyricist=? where SongId=?", song.getLyricist(), song.getSongName());
        }
        if (song.getSinger() != null) {
            db.update("update playlist set singer=? where SongId=?", song.getSinger(), song.getSongName());
        }
